...
def train_test_val_split(df):
    ...
    df_train = df[(df["Date"] >= "2014-01-01") & (df["Date"] <= "2019-12-31")].reset_index(drop=True)
    df_val = df[(df["Date"] >= "2020-01-01") & (df["Date"] <= "2020-12-31")].reset_index(drop=True)
    df_test = df[(df["Date"] >= "2021-01-01") & (df["Date"] <= "2022-12-31")].reset_index(drop=True)
